...
namespace Infrastructure
{
    public class Captcha
    {
        ...
        private Bitmap GetImage()
        {
            ...
            //创建绘图
            result = new Bitmap(this.verifyCodeText.Length * 20, 25);
            ...
            using (Graphics objGraphics = Graphics.FromImage(result))
            {
                ...
                using (SolidBrush objSolidBrush = new SolidBrush(this.fontColor))
                {
                    ...
                    for (int i = 0; i < chars.Length; i++)
                    {
                        ...
                        objGraphics.DrawString(chars[i].ToString(), objFont, objSolidBrush, 8, 2, objStringFormat);
                        objGraphics.RotateTransform(-angle);
